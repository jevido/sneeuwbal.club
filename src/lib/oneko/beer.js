export function oneko() {
	...
	function init() {
		...
		let nekoFile = './cat_to_beer_sprite.gif';
		...
		nekoEl.addEventListener('dblclick', performSpecialAction);
	...
	function idle() {
		...
		if (idleTime > 10 && Math.floor(Math.random() * 200) == 0 && idleAnimation == null) {
			let availableIdleAnimations = ['sleeping', 'scratchSelf', 'play'];
			if (nekoPosX < 32) {
	...
	function performSpecialAction() {
		// Add a special double-click action
		idleAnimation = 'play';
		idleAnimationFrame = 0;
		explodeHearts();
	}
